...
#   - A uv-managed Python virtual environment with FastAPI + Uvicorn.
#
...
if [ -f "$PYPROJECT" ]; then
  ...
  log "No requirements.txt/pyproject.toml found; installing FastAPI stack directly..."
  VIRTUAL_ENV="$VENV_DIR" uv pip install \
    "fastapi" "uvicorn[standard]" "pydantic" "python-multipart" "pyzmq"
fi
